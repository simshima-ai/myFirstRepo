export function bindKeyboardInput(state, helpers, deps) {
    ...
    const onKeyDown = (e) => {
        ...
        if (isEnterKey(e) && state.tool === "fillet") {
            const canCommitBySelection = state.selection.ids.length === 2;
            const hasPendingStep = !!state.input?.filletFlow;
            if (canCommitBySelection || hasPendingStep) {
                const hint = state.input?.hover?.world || state.input?.hoverWorld || null;
                const committed = commitFilletFromHover(hint);
                if (committed) {
                    const stillPendingStep = !!state.input?.filletFlow;
                    if (!stillPendingStep) {
                        clearSelection();
