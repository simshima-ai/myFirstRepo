  ...
  async function loadAppSettingsAtStartup() {
    try {
      if (isProjectFolderApiSupported()) {
        const dirHandle = await loadProjectDirectoryHandle();
        if (dirHandle) {
          const permission = await queryProjectDirectoryPermission(dirHandle, false);
          if (permission === "granted") {
            const fileData = await readProjectSettingsFile(dirHandle);
            if (applyLoadedSettings(fileData)) return true;
          }
        }
      }
      ...
      if (!raw) return false;
      return applyLoadedSettings(JSON.parse(raw));
    } catch (_) {
      return false;
    }
  }
