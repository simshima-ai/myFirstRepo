import { worldToScreen, screenToWorld, getEffectiveGridSize, mmPerUnit } from "./geom.js";
...
function drawShape(ctx, state, shape, currentShapeGroupMap = null) {
  ...
  const baseStroke = groupColorize
    ...
    : (layerColorize ? getLayerColorById(state, shape.layerId) : "#0f172a");
...
export function render(ctx, canvas, state) {
  ...
  for (const shape of state.shapes) drawShape(ctx, state, shape, currentShapeGroupMap);
